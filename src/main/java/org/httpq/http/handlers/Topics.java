...
public class Topics extends HttpHandler{
  private final PgPool pool;

  @Override
  Set<SecurityKeyScope> getRequiredScope() {
    return Set.of(SecurityKeyScope.TENANT_MASTER);
  }

  public Topics(PgPool pool) {
    this.pool = pool;
  }
  ...
  public void list(RoutingContext ctx) {
    ...
    pool.withConnection(conn -> new Repositories(conn).topic().list(securityKey.tenantId()).onComplete(ar -> {
      if (ar.succeeded()) {
  ...
  public void create(RoutingContext ctx) {
    ...
    String version = o.getString("name");

    pool.withConnection(conn -> new Repositories(conn).topic().create(securityKey.tenantId(), version).onComplete(ar -> {
      if (ar.succeeded()) {
        ctx.response()
          .setStatusCode(201)
