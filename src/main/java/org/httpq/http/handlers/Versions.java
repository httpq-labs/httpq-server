...
public class Versions extends HttpHandler{
  ...
  public void list(RoutingContext ctx) {
    ...
    pool.withConnection(conn -> new Repositories(conn).version().list(securityKey.tenantId()).onComplete(ar -> {
      if (ar.succeeded()) {
  ...
  public void create(RoutingContext ctx) {
    ...
    pool.withConnection(conn -> new Repositories(conn).version().create(securityKey.tenantId(), version).onComplete(ar -> {
      if (ar.succeeded()) {
        ctx.response()
          .setStatusCode(201)
